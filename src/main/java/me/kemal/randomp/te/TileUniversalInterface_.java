...
public class TileUniversalInterface_ extends TileEnergyStorage implements ISidedInventory, IFluidHandler {
	...
	public TileUniversalInterface_() {
		super(capacity);
		heldStack = null;
		allowAutoInput = true;
		...
		peripheral.setDescription(
				"The Universally Interface in an Computer controlled proxy for item, fluids and energy");
		peripheral
				...
				this);
		peripheral.AddMethod("pushStack", "Pushs the currently holded stack into an external inventory",
				new CCType[] {
						...
						new CCType(String.class, "simulatedDir",
								"Only used if you want to simulate that the item was push from an specific direction, if you don't simply insert this argument an empty string then it will use the real direction from where the items come(valid inputs: [an empty string], front, back, right, left, top, bottom)") },
				new CCType[] { new CCType(HashMap.class,
						"The new inventory content or nil, if it is empty") },
				this);
		peripheral.AddMethod("suckStack",
				...
				new CCType[] {
						...
						new CCType(String.class, "simulatedDir",
								"Only used if you want to simulate that the item was pulled from an specific direction, if you don't simply insert this argument an empty string then it will use the real direction from where the items come(valid inputs: [an empty string], front, back, right, left, top, bottom)") },
				new CCType[] { new CCType(HashMap.class, "The pulled stack, or nil if nothing could be pulled") },
				this);
	...
	public void updateEntity() {
		...
		for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
			storedEnergy.setMaxExtract(getMaxEnergyOutput(dir.ordinal()));
			storedEnergy.setMaxReceive(getMaxEnergyInput(dir.ordinal()));
			...
			/*if (neightborCache[(dir.ordinal())] != null && neightborCache[(dir.ordinal())] instanceof IFluidHandler
					&& this.tank.getFluid() != null && (getIOConfiguration(dir.getOpposite().ordinal()) == SIDE_IO
							|| getIOConfiguration(dir.getOpposite().ordinal()) == SIDE_FLUID_ONLY)) {
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context, String method, Object[] arguments,
		...
		case "pushStack": {
			...
			int simulatedDir = -1;
			if (arguments[1] != "")
				simulatedDir = Util.relDirToAbsDir(facing, Util.readableRelDirToRelForgeDir((String) arguments[1]));
			else
	...
	public boolean decrSide(int side) {
		...
		ioConfiguration[BlockHelper.ICON_ROTATION_MAP[facing][side]] = (ioConfiguration[BlockHelper.ICON_ROTATION_MAP[facing][side]]
				- 1 > -1) ? ioConfiguration[BlockHelper.ICON_ROTATION_MAP[facing][side]] - 1 : 3;
		updateAllBlocks();
		return true;
	}
	...
	public boolean canFill(ForgeDirection from, Fluid fluid) {
		return tank.getCapacity() == tank.getFluidAmount() && (getIOConfiguration(from.ordinal()) == SIDE_FLUID_ONLY

...
import me.kemal.randomp.computercraft.BlockSourceImplDispenserHack;
...
public class TurtleUpgradeDispense extends RandomPTurtleUpgrade {
	...
	public boolean dispense(int dir, ITurtleAccess turtle) {
		if (dir != -1) {
			BlockSourceImpl blocksourceimpl = new BlockSourceImplDispenserHack(turtle.getWorld(), dir,
					turtle.getPosition().posX, turtle.getPosition().posY, turtle.getPosition().posZ);
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context, String method, Object[] arguments,
		...
		if (method == "dispense") {
			int dir = Util.turtleDirToAbsDir(turtle.getDirection(), (String) arguments[0]);
			if (dir != -1) {
